...
export default function OrderModal({ isOpen, onClose, stock, initialAction, currentPrice }) {
  ...
  const [stopLossPriceType, setStopLossPriceType] = useState('LIMIT');
  ...
  const effectivePrice = priceType === 'MARKET' ? currentPrice : price;
  const approxMargin = quantity * (effectivePrice || 0);

  const handleConfirmRequest = (e) => {
    e.preventDefault();
    if (productType === 'INTRADAY') {
      toast({ message: "Intraday trading is not supported yet.", type: "warning" });
      return;
    }
    setConfirming(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ message: `${action} order placed for ${stock?.symbol || 'stock'} · ₹${approxMargin.toLocaleString('en-IN', { maximumFractionDigits: 2 })}`, type: "success" });
      onClose();
    } catch (error) {
      console.error("Order creation failed:", error);
      toast({ message: error?.response?.data?.error || "Failed to place order.", type: "error" });
      setConfirming(false);
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 font-sans backdrop-blur-sm transition-all">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col">
        ...
        {confirming ? (
          <div className="p-5 flex flex-col gap-4">
            <div className="text-center">
              <CheckCircle size={36} style={{ color: themeColor, margin: "0 auto 0.5rem" }} />
              <div className="font-bold text-gray-800 text-base">Confirm Order</div>
              <div className="text-xs text-gray-500 mt-0.5">Review your order before placing</div>
            </div>

            <div className="bg-gray-50 rounded-xl border border-gray-100 overflow-hidden">
              {[
                ["Action",       <span style={{ color: themeColor, fontWeight: 700 }}>{action}</span>],
                ["Stock",        stock?.symbol],
                ...
                ["Price",        priceType === 'MARKET' ? "Market Price" : `₹${Number(price).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`],
                ...(orderType === 'STOPLOSS' ? [
                  ["Trigger Price", `₹${Number(triggerPrice).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`],
                  ["SL Price",      stopLossPriceType === 'MARKET' ? "Market" : `₹${Number(stopLossPrice).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`],
                ] : []),
                ["Approx Value", `₹${approxMargin.toLocaleString('en-IN', { maximumFractionDigits: 2 })}`],
              ].map(([label, val], i) => (
                <div key={i} className="flex justify-between items-center px-3.5 py-2.5 border-b border-gray-100 last:border-0">
            ...
            <div className="flex gap-2 mt-1">
              ...
              <button
                ...
                disabled={submitting}
                className="flex-[2] py-3 rounded-xl text-white font-bold text-sm tracking-widest uppercase shadow-lg transition-all hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-60 disabled:cursor-not-allowed"
                style={{ backgroundColor: themeColor, boxShadow: `0 4px 14px ${themeColor}40` }}
              >
                {submitting ? "Placing…" : `Confirm ${action}`}
              </button>
            </div>
          </div>
        ) : (
          <div className="p-5 flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-4 mt-1">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:border-gray-400 focus:ring-1 focus:ring-gray-400"
                  />
                  <div className="text-[10px] text-gray-400 mt-1">Triggers when price reaches this value.</div>
                </div>
                <div>
                  <div className="flex justify-between items-center mb-1.5">
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider">SL Price</label>
                    <div className="flex items-center gap-2 text-[0.65rem] font-semibold bg-gray-100 p-0.5 rounded">
                      <button
                  ...
                  />
                  <div className="text-[10px] text-gray-400 mt-1">Executes order at this price.</div>
                </div>
              </div>
            )}
            ...
            <div className="flex justify-between items-center bg-gray-50 p-3.5 rounded-xl mt-2 border border-gray-100">
              <span className="text-xs text-gray-500 font-semibold uppercase tracking-wider">Approx Value</span>
        ...
        {/* Place Order Button — only shown on form step */}
        {!confirming && (
